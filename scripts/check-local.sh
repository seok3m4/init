...
verify_ownership() {
  ...
  case "$ROLE" in
    ...
    B) pattern="($common|$baseline|^frontend/src/features/company-recruiting/|^frontend/src/app/(layout\.tsx|page\.tsx|company/recruitments/|company/applicants/|company/applications/)|^frontend/src/styles/|^frontend/public/logo-init\.png$|^frontend/(next-env\.d\.ts|next\.config\.ts|tsconfig\.json)$|^backend/api/(src|prisma)/|^backend/api/tsconfig(\.build)?\.json$|^docs/03_contracts/|^docs/02_architecture/)" ;;
