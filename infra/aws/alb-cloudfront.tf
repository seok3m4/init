...
resource "aws_cloudfront_origin_access_control" "assets" {
  ...
  description                       = "OAC for private S3 asset bucket"
...
resource "aws_cloudfront_distribution" "app" {
  ...
  ordered_cache_behavior {
    path_pattern           = "/company/*/profile-logo/*"
  ...
  ordered_cache_behavior {
    path_pattern           = "/company/*/jd-images/*"
    target_origin_id       = "assets-s3"
    viewer_protocol_policy = "redirect-to-https"
    allowed_methods        = ["GET", "HEAD", "OPTIONS"]
    cached_methods         = ["GET", "HEAD"]
...
data "aws_iam_policy_document" "assets_bucket" {
  statement {
    sid     = "AllowCloudFrontReadForPublicAssetPrefixes"
    effect  = "Allow"
