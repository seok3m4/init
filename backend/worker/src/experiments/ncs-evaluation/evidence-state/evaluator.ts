...
const RESULT_PATTERN = /(줄었|내려갔|단축|개선됐|정상화|발견해\s*수정|발견했고|합의해|합의하여|승인을\s*받|예정일에\s*배포|문제없이\s*배포|확정했습니다|더\s*이상\s*발생하지|돌아왔|오류를\s*줄였|전달했습니다|결함을\s*찾아\s*수정|다시\s*만들|장애\s*시간이\s*더\s*길어졌|결제\s*오류가\s*발생|결과\s*지표|결과를\s*확인|개선\s*결과|증가했습니다|(?:차단|방지|감소|증가|유지|해소)(?:됐|되었|되고|하지|했습니다|했고).{0,60}(?:확인|검증|측정))/u;
