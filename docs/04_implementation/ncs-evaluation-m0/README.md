...
- 팀별 브랜치와 수정 가능 경로가 충돌하지 않게 정의되어 있다.
...
공용 자산 변경이 필요하면 별도 제안으로 올리고 PM, C, D, E가 함께 검토한다.

- PM: 공통 계약과 비교 기준 승인
- C: 행동 포인트와 질문 적합성 검토
- D: 면접 답변·꼬리질문 입력 계약 검토
- E: 평가기, worker 실행, 결과 형식 검토
- A: 모델 secret, 비용·실행 환경 검토
