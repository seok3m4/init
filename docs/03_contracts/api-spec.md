...
- 비고/미결:
  ...
  - B는 `/public/applications/{applicationId}/interview?token={token}` 브릿지 화면까지만 제공한다.
  - 브릿지 화면은 매직링크 token으로 API-089를 다시 검증하고, 실제 D 면접 API는 호출하지 않는다.
  - 실제 면접 시작은 D의 public interview access context/guard 확정 후 연결한다.
