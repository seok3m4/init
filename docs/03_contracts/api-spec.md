...
- 검증/전제조건:
  ...
  - JD 파일 업로드는 이번 API에서 처리하지 않고, `jobDescription` 텍스트만 저장한다.
- 성공 응답/처리:
...
- 검증/전제조건:
  - 이메일 형식이 유효해야 함
  - 메모는 B 소유 `applications.screening_memo`에만 저장한다.
- 성공 응답/처리:
  - 지원자 등록 완료
- 오류/예외:
...
- 비고/미결:
  ...
  - 연락처는 `users.phone`, 메모는 `applications.screening_memo` 기준으로만 저장한다. D/E 소유 평가/면접/리포트 필드는 write하지 않는다.
...
- 비동기: Y
- DTO:
  - Request DTO: `SuggestEvaluationCriterionDto`
  - Response DTO: `EvaluationCriterionSuggestionProcessResponseDto`
- 요청 데이터:
  - `postingId: number`
  - `jobRole?: string`
  - `jdText?: string`
...
- 검증/전제조건:
  ...
  - `requestedCount`는 양수이며 API별 최대 허용 개수는 구현 시 정책값으로 제한한다.
- 성공 응답/처리:
  - 평가 역량 후보 표시
  - Response envelope: `{ data, meta }`
...
- Request Body:
  - `postingId`: number, required
  - `criterionIds`: number[], optional
  - `questionTypes`: `QuestionType[]`, optional
  - `requestedCount`: number, optional, 1~20
...
- Request Body:
  - `postingId`: number, required
  - `questionIds`: number[], optional
  - `questionTypes`: `QuestionType[]`, optional
  - `questionCount`: number, required, 1~20
- Response Body:
  - `postingId`: number
  - `questionSet`: `{ questionIds, questionCount, readyForSession }`
- Validation:
  - 평가 기준과 활성 질문 뱅크가 존재해야 한다.
  - `questionIds`가 있으면 모두 같은 공고의 활성 질문이어야 한다.
  - 요청한 질문 수보다 사용 가능한 질문이 부족하면 저장하지 않는다.
- Error Codes:
