...
- 비고/미결:
  ...
  - JD 이미지 파일 업로드/S3/file_assets 저장은 이번 API에서 처리하지 않고, 에디터 내 이미지 URL 삽입만 프론트 임시 범위로 허용한다.
...
- 검증/전제조건:
  ...
  - JD 파일 업로드는 이번 API에서 처리하지 않고, `jobDescription` rich text HTML 문자열만 저장한다.
- 성공 응답/처리:
...
- 관련 ERD 테이블:
  - companies, postings
- 비고/미결:
  - JD 파일 업로드 UI는 제공할 수 있으나 실제 파일 저장/S3/file_assets/텍스트 추출 API는 후속 합의가 필요하다.
  - 에디터 이미지 업로드는 후속 합의 전까지 URL 삽입만 허용한다.
...
- 비고/미결:
  - 실제 Redis/SMTP/Auth magic link 발급은 이번 API에서 직접 수행하지 않는다.
  - B 모듈 내부 adapter 경계로 요청 결과만 반환하고, 실제 인증/메일 발송은 A/Auth 연동 후 교체한다.
  - 파일 업로드/S3/file_assets/application_documents 저장은 이번 범위에서 제외한다.

### API-088 POST /public/recruitments/{recruitmentId}/applications/status
- 도메인: 공개 - 채용공고/지원
- 권한/인증: 비로그인 허용
- 관련 화면: 공개 지원 현황 화면 (/public/recruitments/{recruitmentId}/applications/status)
- UI Type: form/detail
- 상태 코드: 200 OK
- 비동기: N
- Path Params:
...
- 검증/전제조건:
  ...
  - 기업 내부 판정값, 내부 메모, 평가 리포트 상세, 점수, 근거는 공개 응답에 포함하지 않는다.
  - `ARCHIVED` 공고의 지원 현황은 공개 조회하지 않는다.
- 성공 응답/처리:
  - 지원자가 본인 지원 현황과 면접 준비 상태를 확인할 수 있는 제한된 데이터를 `{ data, meta }` envelope로 반환한다.
  - 응답 필드:
    - `applicationId`
    - `recruitmentId`
    - `candidateName`
    ...
    - `recruitment.jobRole`
    - `recruitment.status`
    - `recruitment.startsOn`
    - `recruitment.endsOn`
    - `statuses.applicationStatus`
    - `statuses.documentStatus`
    - `statuses.interviewStatus`
    - `statuses.reportStatus`
    - `interviewAccess.status`
    - `interviewAccess.nextAction`
    - `interviewAccess.sessionId`
    - `interviewAccess.interviewType`
    - `interviewAccess.temporary`
    - `interviewAccess.temporaryBoundary`
- 오류/예외:
  - 이메일 형식 오류는 `COMMON_VALIDATION_FAILED`를 반환한다.
  - 지원 내역이 없거나 공개 조회할 수 없는 공고면 `COMMON_NOT_FOUND`를 반환한다.
- 관련 ERD 테이블:
  - companies, postings, users, candidate_profiles, applications, interview_sessions, evaluation_reports
- 비고/미결:
  - 실제 이메일 인증/매직링크 토큰 검증은 이번 API에서 직접 수행하지 않는다.
  - B 모듈의 임시 이메일 매칭 방식으로 제한된 지원 현황을 보여주며, A/Auth의 Redis/SMTP magic link 연동 후 토큰 기반 조회로 교체한다.
  - D 소유 면접 런타임 URL은 이번 범위에서 생성하지 않는다. 면접 세션이 있으면 상태와 sessionId만 노출한다.
...
- 비고/미결:
  ...
  - `timePolicy` 저장 위치는 DB 문서상 아직 명확하지 않으므로 실제 저장 구현은 A/PM/D 리뷰 필요
