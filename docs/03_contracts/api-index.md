...
| `backend/api/src/modules/company-recruiting` | `CompanyRecruitingController`, `PublicRecruitmentController`, `PublicApplicationController` | `/api/v1/company/recruitments`, `/api/v1/company/applicants`, `/api/v1/public/recruitments`, `/api/v1/public/applications` | B | API-010..033 중 공고/지원자 운영, API-080, API-085..089 |
...
| API-015 | 기업 - 지원자/리포트 | POST | /company/applicants | 지원자 직접 등록 | 기업 / 기업 사용자 로그인 | N | 201 Created |
| API-085 | 기업 - 지원자/리포트 | POST | /company/applicants/bulk | 지원자 CSV 일괄 등록 | 기업 / 기업 사용자 로그인 | N | 200 OK |
...
| API-089 | 공개 - 채용공고/지원 | GET | /public/applications/status | 매직링크 토큰 기반 지원 현황 조회 | 매직링크 토큰 필요 | N | 200 OK |
| API-089B | 공개 - 채용공고/지원 | POST | /public/applications/token/verify | D public 면접 시작용 매직링크 토큰 검증 | B-D 서버 내부 호출 / 매직링크 토큰 필요 | N | 200 OK |
...
| API-089 | 지원자 - Public 채용면접 | GET | /public/applications/{applicationId}/interview | public 채용면접 런타임 조회 | publicAccessToken | N | 200 OK |
